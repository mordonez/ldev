...
- **Skill**: An agent skill that teaches an agent the correct workflow for a Liferay task (deploying, troubleshooting, resource workflows, etc.). Skills live in `templates/ai/project/skills/` and are the primary agent integration surface — they encode intent and sequencing that raw CLI output cannot express. Projects obtain them by copying them manually or via `npx skills add https://github.com/mordonez/ldev`.
...
src/
  ...
  entrypoints/  Long-lived server processes: dashboard (HTTP)
...
### Zod contracts — source of truth for structured CLI output and dashboard routes

`src/core/contracts/`. All `--json` CLI outputs and dashboard API responses have explicit Zod schemas. TypeScript types are derived from schemas with `z.infer<>`. Import from `src/core/contracts/index.ts`. See [ADR 0003](docs/adr/0003-zod-contracts-in-core.md).
...
- `src/core/contracts/index.ts`: all Zod schemas for structured CLI output and dashboard routes.
...
- `src/features/agent/`: `ldev ai bootstrap` — runtime agent context generation.
- `templates/ai/`: skills and project templates; `docs/ai/` for copy-paste agent meta-files.
