...
- **Skill**: An agent skill installed by `ldev ai install` that teaches an agent the correct workflow for a Liferay task (deploying, troubleshooting, resource workflows, etc.). Skills are the primary agent integration surface — they encode intent and sequencing that raw CLI output cannot express.
