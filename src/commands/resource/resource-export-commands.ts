...
import {configure, o, registerResourceWorkflow, requireResourceValue} from './resource.command.js';
...
export function registerResourceExportCommands(resource: Command): void {
  ...
  registerResourceWorkflow(resource, {
    ...
    configure: configure(
      o.site(),
      (cmd) => cmd.option('--dir <dir>', 'Destination base directory; defaults to paths.structures'),
      o.allSites('Export structures for every accessible site'),
      o.checkOnly(),
    ),
    run: async (context, options) =>
  ...
  registerResourceWorkflow(resource, {
    ...
    configure: configure(
      o.site(),
      (cmd) => cmd.option('--dir <dir>', 'Destination base directory; defaults to paths.templates'),
      o.allSites('Export templates for every accessible site'),
      (cmd) =>
        cmd.option('--debug', 'Show which enumeration source was used and how many templates each source returned'),
      o.continueOnError('template'),
    ),
    run: async (context, options) =>
  ...
  registerResourceWorkflow(resource, {
    ...
    configure: configure(
      ...
      o.allSites('Export ADTs for every accessible site'),
      o.continueOnError('ADT'),
    ),
    run: async (context, options) =>
  ...
  registerResourceWorkflow(resource, {
    ...
    configure: configure(o.site(), o.allSites('Export fragments for every accessible site'), (cmd) =>
      cmd
        .option('--dir <dir>', 'Destination project directory; defaults to paths.fragments/sites/<site>')
