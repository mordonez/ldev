import {resolveRuntimeNumberConfig, resolveRuntimeStringConfig} from './liferay-runtime-config.js';

export type ResolvedLiferayConfigInput = {
  processEnv: NodeJS.ProcessEnv;
  dockerEnv: Record<string, string>;
  localProfile: Record<string, string>;
  /** Fallback value for scopeAliases when not configured explicitly. */
...
export function resolveLiferayConfig(input: ResolvedLiferayConfigInput) {
  ...
  return {
    ...
    scopeAliases: resolveRuntimeStringConfig({
      ...
      fallbackValue: input.scopeAliasDefault,
    }),
