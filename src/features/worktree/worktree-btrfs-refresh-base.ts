...
export async function runWorktreeBtrfsRefreshBase(options: {
  ...
  if (!config.repoRoot) {
    throw new CliError('worktree btrfs-refresh-base requiere ejecutarse dentro de un repositorio válido.', {
  ...
  if (!btrfs.baseDir) {
    throw new CliError('BTRFS_BASE no resuelta tras la validación de configuración.', {
