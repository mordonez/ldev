...
export async function runLiferayResourceSyncAdt(
  ...
  if (!existing) {
    if (!options.createMissing) {
      throw new CliError(`ADT '${name}' no existe y create-missing no está activo.`, {
        code: 'LIFERAY_RESOURCE_ERROR',
      });
    }
...
function inferAdtName(file: string): string {
  if (!file) {
    throw new CliError(
      "ADT requiere --file o (--key y --widget-type). Usa 'resource resolve-adt --display-style ddmTemplate_<ID>' si necesitas resolverlo primero.",
      {code: 'LIFERAY_RESOURCE_ERROR'},
    );
  }
