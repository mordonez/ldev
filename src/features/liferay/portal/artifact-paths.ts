...
async function resolveAdtArtifactFile(config: AppConfig, key: string, widgetType: string): Promise<string> {
  if (!config.paths?.adts) {
    throw createConfigIncompleteError('ADT', key);
  }
...
async function resolveExistingArtifactFile(config: AppConfig, fileOverride: string): Promise<string> {
  ...
  for (const baseDir of [
    ...
    const nested = path.resolve(baseDir, fileOverride);
    if (await fs.pathExists(nested)) {
      return nested;
    }
  }

  throw LiferayErrors.resourceFileNotFound(fileOverride, {
    details: {candidate: fileOverride},
  });
}

function detectFragmentsProjectRoot(startPath: string): string | undefined {
