...
async function readDescriptorNode(config: AppConfig, migrationFile: string): Promise<Record<string, unknown>> {
  ...
  if (!(await fs.pathExists(candidate))) {
    throw new CliError(`Descriptor de migración no encontrado: ${migrationFile}`, {code: 'LIFERAY_RESOURCE_ERROR'});
  }
...
function parseMigrationDescriptor(descriptorNode: Record<string, unknown>): MigrationDescriptor {
  ...
  if (structureKey === '') {
    throw new CliError('Descriptor inválido: falta structureKey', {code: 'LIFERAY_RESOURCE_ERROR'});
  }

  const introduceNode = asRecord(descriptorNode.introduce);
  if (Object.keys(introduceNode).length === 0) {
    throw new CliError('Descriptor inválido: falta introduce.', {code: 'LIFERAY_RESOURCE_ERROR'});
  }
...
function parseStageDescriptor(node: Record<string, unknown>, stageName: MigrationStage): MigrationStageDescriptor {
  ...
  const mappings = Array.isArray(node.mappings) ? node.mappings : asRecord(node.plan).mappings;
  const planNode = {mappings: normalizeMappingsArray(mappings)};
  if (!Array.isArray(planNode.mappings) || planNode.mappings.length === 0) {
    throw new CliError(`Descriptor inválido: ${stageName}.mappings[] vacío.`, {code: 'LIFERAY_RESOURCE_ERROR'});
  }
  return {
    structureFile,
    planNode,
...
async function runCleanupStage(
  ...
  const cleanupPlanFile = await writeTempPlanFile({
    mappings: normalizeMappingsArray(descriptor.introduce.planNode.mappings)
    ...
    ...(migratedArticleKeys.length > 0 ? {articleIds: migratedArticleKeys} : {}),
  });
