...
export async function runLiferayResourceMigrationInit(
  ...
  const removedFieldReferences = [...setDifference(
    collectFieldReferences(structure.raw),
    collectFieldReferences(localStructure),
  )].sort();
  const candidateTargetFieldReferences = [...collectMigrationTargets(localStructure)].sort();
  const outputPath = resolveMigrationDescriptorOutputPath(config, options.output, structure.siteFriendlyUrl, structure.key);
  ...
  const descriptor = {
    ...
    introduce: {
      ...
      mappingHelp: {
        ...
        target: 'Field reference de destino en la estructura nueva. Si cae dentro de un fieldset, usa fieldset[].campo.',
        cleanupSource: 'Ponlo a true si el campo origen se debe limpiar en la segunda fase y desaparecer de la estructura final.',
        examples: [
...
function resolveMigrationDescriptorOutputPath(
  ...
  if (output) {
    return path.isAbsolute(output)
      ? output
      : path.resolve(config.repoRoot ?? config.cwd, output);
  }
...
function collectReferencedDependentStructures(definition: Record<string, unknown>, mainStructureKey: string): Set<string> {
...
function collectReferencedDependentStructuresRecursive(
  ...
  for (const field of fields) {
    ...
    if (String(record.fieldType ?? '').trim().toLowerCase() === 'fieldset' && fieldsetKey !== '' && fieldsetKey !== mainStructureKey) {
...
function collectMigrationTargetsRecursive(fields: unknown, targets: Set<string>, fieldsetName: string | undefined): void {
  if (!Array.isArray(fields)) {
    return;
  }
  for (const field of fields) {
...
function isFieldsetField(field: Record<string, unknown>): boolean {
  return String(field.fieldType ?? '').trim().toLowerCase() === 'fieldset';
}
