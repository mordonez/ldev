...
export function requireRepoRoot(config: AppConfig): string {
  if (!config.repoRoot) {
    throw new CliError('Este comando requiere ejecutarse dentro de un repositorio del proyecto.', {
...
export async function resolveStructureFile(config: AppConfig, key: string, file?: string): Promise<string> {
  ...
  throw new CliError(`Structure file no encontrado para ${key} en ${baseDir}. Usa --file.`, {
...
export async function resolveTemplateFile(
  ...
  throw new CliError(`Template file no encontrado para ${name} en ${baseDir}. Usa --file.`, {
    code: 'LIFERAY_RESOURCE_FILE_NOT_FOUND',
  });
}
...
export async function resolveAdtFile(
  ...
  throw new CliError(`ADT file no encontrado para ${name} (${widgetType}) en ${baseDir}. Usa --file.`, {
    code: 'LIFERAY_RESOURCE_FILE_NOT_FOUND',
  });
}
...
async function resolveExistingResourceFile(config: AppConfig, candidate: string): Promise<string> {
  ...
  throw new CliError(`File no encontrado: ${candidate}`, {
    code: 'LIFERAY_RESOURCE_FILE_NOT_FOUND',
  });
}
