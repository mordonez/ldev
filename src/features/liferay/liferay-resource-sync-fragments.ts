...
export async function runLiferayResourceSyncFragments(
  ...
  if (options?.allSites && (options?.fragment ?? '').trim() !== '') {
    throw new CliError('--fragment requiere --site o --site-id', {
...
async function readLocalFragmentsProject(projectDir: string, fragmentFilter: string): Promise<LocalFragmentsProject> {
  ...
  if (!(await fs.pathExists(srcDir))) {
    throw new CliError(`No existe directorio src en ${projectDir}`, {
      code: 'LIFERAY_RESOURCE_ERROR',
    });
  }
  ...
  if (collections.length === 0) {
    if (filter !== '') {
      throw new CliError(`Fragment '${filter}' no encontrado en ${projectDir}`, {
        code: 'LIFERAY_RESOURCE_ERROR',
      });
    }
    throw new CliError(`No se encontraron fragments para importar en ${projectDir}`, {
      code: 'LIFERAY_RESOURCE_ERROR',
    });
  }
