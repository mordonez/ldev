...
export async function runDbFilesMount(
  ...
  if (!config.dockerDir || !config.files.dockerEnv) {
    throw new CliError('db files-mount requiere ejecutarse dentro de un proyecto con docker/.', {
  ...
  if (nasIp !== '') {
    if (nasShare === '' || nasUser === '') {
      throw new CliError('Con DOCLIB_NAS_IP se requieren DOCLIB_NAS_SHARE y DOCLIB_NAS_USER.', {
...
export function formatDbFilesMount(result: DbFilesMountResult): string {
  if (result.mode === 'local') {
    return `Doclib volume ${result.volume} montado desde ruta local: ${result.path}`;
  }
  if (result.mode === 'nas') {
    return `Doclib volume ${result.volume} montado desde NAS: ${result.path}`;
  }
  return `Doclib volume ${result.volume} listo`;
}
...
async function createLocalBindVolume(volume: string, localPath: string): Promise<void> {
  ...
  if (!result.ok) {
    throw new CliError(result.stderr.trim() || result.stdout.trim() || `No se pudo crear el volumen ${volume}`, {
...
async function createNasVolume(
  ...
  if (!result.ok) {
    throw new CliError(result.stderr.trim() || result.stdout.trim() || `No se pudo crear el volumen CIFS ${volume}`, {
      code: 'DB_DOCLIB_VOLUME_ERROR',
    });
  }
}
