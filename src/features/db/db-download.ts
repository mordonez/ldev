...
export async function runDbDownload(
  ...
  if (!config.dockerDir || !config.files.dockerEnv) {
    throw new CliError('db download must be run inside a project with docker/.', {
...
async function ensureLcpAvailable(): Promise<void> {
  ...
  if (!version.ok) {
    ...
    if (detail === '' || /not found/i.test(detail)) {
      throw new CliError('lcp is not available. Install the LCP CLI or use a local backup with db import.', {
...
async function resolveLatestBackupId(project: string, environment: string): Promise<string> {
  ...
  if (!result.ok) {
    throw new CliError(
      `Could not list backups for ${project}/${environment}: ${result.stderr.trim() || result.stdout.trim()}`,
      {
...
async function ensureDatabaseBackup(
  ...
  await runStep(printer, 'Downloading database backup from LCP', async () => {
    const result = await runProcess(
      'lcp',
      [
        'backup',
