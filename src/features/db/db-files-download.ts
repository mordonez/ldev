...
export async function runDbFilesDownload(
  ...
  if (!config.dockerDir || !config.files.dockerEnv) {
    throw new CliError('db files-download requiere ejecutarse dentro de un proyecto con docker/.', {
...
async function ensureLcpAvailable(): Promise<void> {
  ...
  if (!version.ok) {
    ...
    if (detail === '' || /not found/i.test(detail)) {
      throw new CliError('lcp no disponible. Instala LCP CLI para descargar doclib.', {
...
async function ensureDoclibBackup(options: {
  ...
  await runStep(options.printer, 'Descargando doclib desde LCP', async () => {
    const result = await runProcess(
      'lcp',
      ['backup', 'download', '--project', options.project, '--environment', options.environment, '--backupId', options.backupId, '--doclib', '--dest', targetDoclibDir],
      {reject: false},
    );
    if (!result.ok) {
...
async function ensureDoclibBackupBackground(options: {
  ...
  for (let waitCount = 0; waitCount < 120; waitCount += 1) {
    const doclibDir = await findDirectoryNamed(targetDoclibDir, 'doclib', 3);
    if (doclibDir) {
      const resolvedDoclibRoot = await resolveDoclibRoot(doclibDir);
      const currentContent = await fs.readFile(options.dockerEnvFile, 'utf8').catch(() => '');
      const updatedContent = upsertEnvFileValues(currentContent, {DOCLIB_PATH: resolvedDoclibRoot});
      await fs.writeFile(options.dockerEnvFile, `${updatedContent}\n`);
    ...
    if (!processExists(child.pid ?? -1)) {
      await fs.remove(pidFile);
      throw new CliError(`La descarga falló al arrancar. Revisa: ${logFile}`, {
        code: 'DB_LCP_DOCLIB_DOWNLOAD_FAILED',
      });
    }
  ...
  await fs.remove(pidFile);
  throw new CliError(`Descarga de doclib no comenzó en tiempo límite. Revisa: ${logFile}`, {
