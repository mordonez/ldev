...
export function formatEnvInit(result: EnvInitResult): string {
  const lines = [`Entorno inicializado en ${result.dockerEnvFile}`];
  if (result.created) {
    lines.push('Se ha creado docker/.env.');
  }
  if (result.mergedKeys.length > 0) {
    lines.push(`Claves añadidas desde .env.example: ${result.mergedKeys.join(', ')}`);
  }
  return lines.join('\n');
}
