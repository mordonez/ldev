...
3. verify logs with `ldev logs --no-follow`
4. use `ldev portal inventory ... --json` when the change affects page behavior or content rendering

Use Blade as the underlying Workspace tool when needed, but prefer `ldev` as the high-level command surface when it already supports the workflow.
