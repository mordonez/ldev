...
These principles describe what the codebase already does. They exist so reviewers can point to them rather than re-derive them.

1. **One entrypoint per binary** — each binary lives in `src/<name>.ts` and delegates immediately.
