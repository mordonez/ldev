...
describe('env integration', () => {
  ...
  test('env start can copy a local activation key into the project before boot', async () => {
    ...
    expect(startResult.exitCode).toBe(0);
    const parsed = JSON.parse(startResult.stdout);
    expect(parsed.activationKeyFile).toBe(path.join(modulesDir, 'activation-key-sample.xml'));
  ...
  test('env restore rehydrates a worktree from BTRFS_BASE and preserves local deploy cache', async () => {
    ...
    await fs.writeFile(path.join(btrfsBase, 'postgres-data', 'PG_VERSION'), '15\n');
    await fs.writeFile(path.join(btrfsBase, 'liferay-data', 'from-base.txt'), 'base\n');
    ...
    expect(result.sourceDataRoot).toBe(btrfsBase);
