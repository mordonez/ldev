...
describe('cleanup integration', () => {
  test('env clean removes local data root inside repo and requires --force', async () => {
    ...
    const result = await runCli(['env', 'clean', '--force', '--format', 'json'], {
      cwd: repoRoot,
      env,
    });
  ...
  test('worktree clean removes a registered worktree and gc previews stale ones conservatively', async () => {
    ...
    expect((await runCli(['worktree', 'setup', '--name', 'issue-701'], {cwd: repoRoot, env})).exitCode).toBe(0);
    expect((await runCli(['worktree', 'setup', '--name', 'issue-702'], {cwd: repoRoot, env})).exitCode).toBe(0);
    ...
    const missingForce = await runCli(['worktree', 'clean', 'issue-701'], {
      cwd: repoRoot,
      env,
    });
    ...
    const cleanResult = await runCli(
      ['worktree', 'clean', 'issue-701', '--force', '--delete-branch', '--format', 'json'],
      {cwd: repoRoot, env},
    );
    expect(cleanResult.exitCode).toBe(0);
    expect(await fs.pathExists(wt701)).toBe(false);

    const gcPreview = await runCli(['worktree', 'gc', '--days', '7', '--format', 'json'], {
      cwd: repoRoot,
      env,
    });
  ...
  test('worktree clean preserves ENV_DATA_ROOT outside the worktree perimeter', async () => {
    ...
    expect((await runCli(['worktree', 'setup', '--name', 'issue-703'], {cwd: repoRoot, env})).exitCode).toBe(0);
    ...
    const result = await runCli(['worktree', 'clean', 'issue-703', '--force', '--format', 'json'], {
      cwd: repoRoot,
      env,
    });
    expect(result.exitCode).toBe(0);
  ...
  test('worktree clean removes BTRFS_ENVS data roots owned by the target worktree', async () => {
    ...
    expect((await runCli(['worktree', 'setup', '--name', 'issue-704'], {cwd: repoRoot, env})).exitCode).toBe(0);
    ...
    const result = await runCli(['worktree', 'clean', 'issue-704', '--force', '--format', 'json'], {
      cwd: repoRoot,
      env,
    });
    expect(result.exitCode).toBe(0);
  ...
  test('worktree clean can be re-run after the git worktree is already gone and still removes owned BTRFS data', async () => {
    ...
    expect((await runCli(['worktree', 'setup', '--name', 'issue-705'], {cwd: repoRoot, env})).exitCode).toBe(0);
    ...
    const result = await runCli(['worktree', 'clean', 'issue-705', '--force', '--format', 'json'], {
      cwd: repoRoot,
      env,
    });
    expect(result.exitCode).toBe(0);
